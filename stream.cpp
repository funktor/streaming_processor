...
int parse_row(std::string row,
                std::vector<std::string> colnames,
                std::unordered_map<std::string, std::string> col_dtype_map,
                csv_rows &data, 
                int *&index, 
                FilterTree *tree, 
                GroupBy grp, 
                std::unordered_map<std::string, double> &key_values) {
    ...
    int k = 0;
    std::string curr = "";
    std::unordered_map<std::string, std::string> col_vals;
    ...
    if (is_row_valid && grp.group_by_columns.size() > 0) {
        ...
        std::string col_key = "";
        std::vector<std::string> cols = grp.group_by_columns;

        for (int i = 0; i < cols.size()-1; i++) col_key += col_vals[cols[i]] + "|";
        col_key += col_vals[*cols.rbegin()];
        
        double val = 0.0;
        std::string metric = grp.metric_name;

        if (col_dtype_map[grp.summarize_col] == "int" || col_dtype_map[grp.summarize_col] == "float") {
            if (key_values.find(col_key) == key_values.end()) {
                if (metric == "sum") val = 0.0;
                else if (metric == "min") val = INT32_MAX;
                else if (metric == "max") val = INT32_MIN;
                else if (metric == "count") val = 0.0;
            }
            else {
                val = key_values[col_key];
            }

            if (is_number(col_vals[grp.summarize_col])) {
                if (metric == "sum") val += std::stod(col_vals[grp.summarize_col]);
                else if (metric == "min") val = std::min(val, std::stod(col_vals[grp.summarize_col]));
                else if (metric == "max") val = std::max(val, std::stod(col_vals[grp.summarize_col]));
                else if (metric == "count") val += 1.0;
            }

            key_values[col_key] = val;
        }
    }
...
std::vector<std::string> split_inline(
    std::string &s, 
    ...
    while ((pos = s.find(delimiter)) != std::string::npos) {
        ...
        if (token.size() > 0) {
            ...
            int res = parse_row(token, colnames, col_dtype_map, data, index, tree, grp, key_values);
            // if (res == -1) {
...
int recv_from_socket_ssl(
    ...
    while (1) {
        ...
        if (ret_data > 0) {
            ...
            std::vector<std::string> parts = split_inline(msg, sep, colnames, col_dtype_map, data, index, tree, grp, key_values);
...
int query_process(
        std::string query, FilterTree *&mytree, GroupBy &grpby
    ) {
            ...
            if (stage_parts[0] == "filter") {
                ...
                for (std::string colval : column_values) {
                    ...
                    if (colval_split.size() == 2) {
                        ...
                        if (attr == "metric") {
                            grpby.metric_name = vals;
                        }

                        else if (attr == "on") {
                            std::vector<std::string> on_split = split(vals, ",");
                            for (std::string oncol : on_split) {
                                grpby.group_by_columns.push_back(oncol);
                            }
                        }

                        else if (attr == "summarize") {
                            grpby.summarize_col = vals;
                        }

                        else if (attr == "summarize_new") {
                            grpby.summarize_col_new = vals;
                        }
...
int main(int argc, char *argv[]) {
    ...
    std::vector<std::string> colnames; 
    std::unordered_map<std::string, std::string> col_dtype_map;
    std::unordered_map<std::string, double> grp_by_key_values;
    csv_rows data;
    ...
    int *index = new int[1];
    index[0] = -1;
    int recv_res = recv_from_socket_ssl(conn, msgs, "\n", colnames, col_dtype_map, data, index, mytree, grpby, grp_by_key_values);
    ...
    if (grp_by_key_values.size() > 0) {
        data.clear();

        for (auto kv : grp_by_key_values) {
            std::unordered_map<std::string, std::string> col_vals;
            std::string col_key = kv.first;
            double value = kv.second;
            std::vector<std::string> cols = split(col_key, "|");

            for (int j = 0; j < cols.size(); j++) {
                std::string colname = grpby.group_by_columns[j];
                std::string value = cols[j];
                col_vals[colname] = value;
            }

            col_vals[grpby.summarize_col_new] = std::to_string(value);
            data.push_back(col_vals);
        }
    }
